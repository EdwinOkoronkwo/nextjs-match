    ...
    try {
        ...
        const user: User = await prisma.user.create({
            data: {
                ...
                member: {
                    create: {
                        name, description, city, country, dateOfBirth: new Date(dateOfBirth), gender
                    }
                }
            }
        })
...
export async function verifyEmail(token: string): Promise<ActionResult<string>> {
    try {
        ...
        if (hasExpired) {
            console.log("⏰ Token has expired:", existingToken.expires);
...
export async function generateResetPasswordEmail(email: string): Promise<ActionResult<string>> {
    try {
        ...
        if(!existingUser) return {status: 'error', error: 'Email not found'};
